...
class TensileModel():
    def __init__(self,t, nPlies, Lo, Lg, D,
                              wo, wg, r1):
        '''
        ...
        self.wg = wg
        self.r1 = r1
        # calculate r2 on basis of intersecting radii
        self.r2 = (self.D**2 - 2*self.D*self.Lg + self.wo**2 - 4*wo*self.r1 -
                   2*wo*wg + Lg**2 + 4*r1*wg + wg**2)/(4*wo - 4*wg)
        self.meshSize = 1.0

        # Set name
        self.modelName = 'Lg-{}_wo-{}_r1-{}_r2-{}'.format(
            int(Lg), int(wo), int(r1), int(self.r2))
    ...
    def createSpecimenPart(self):
        ...
        partSketch.Line(point1=(self.Lo, self.wo),
                        point2=((self.Lo + self.D) / 2.0, self.wo))
        # gauge length line
        partSketch.Line(point1=((self.Lo - self.Lg) / 2.0,
                                (self.wo / 2.0) + (self.wg / 2.0)),
                        point2=((self.Lo + self.Lg) / 2.0,
                                (self.wo / 2.0) + (self.wg / 2.0)))

        # determine intersection of two arcs
        arcCenter1 = ((self.Lo - self.D) / 2.0, self.wo - self.r1)
        arcCenter2 = ((self.Lo - self.Lg) / 2.0,
                      (self.wo / 2.0 + self.wg / 2.0 + self.r2))
        p1 = Point(arcCenter1)
        c1 = p1.buffer(self.r1).boundary
        l = LineString([arcCenter1, arcCenter2])
        i = l.intersection(c1)
        ipoint = i.coords[0]

        arc1 = partSketch.ArcByCenterEnds(center=arcCenter2,
                                point1=((self.Lo - self.Lg) / 2.0,
                                        (self.wo / 2.0 + self.wg / 2.0)),
                                point2=ipoint,
                                direction=CLOCKWISE)
        arc2 = partSketch.ArcByCenterEnds(center=arcCenter1,
                                point1=((self.Lo - self.D) / 2.0, self.wo),
                                point2=ipoint,
                                direction=CLOCKWISE)
        ...
        partSketch.copyMirror(
            ...
            objectList=(g.findAt(arc1.pointOn), g.findAt(arc2.pointOn)))                        
...
def main(plyThickness, nLayers, Lo, Lg, D, wo, wg, r1):
    # Create model
    mdl = TensileModel(plyThickness, nLayers, Lo, Lg, D, wo, wg, r1)
    ...
    return smax, sif, int(mdl.r2)

if __name__ == '__main__':
    smax, sif, r2 = main(0.2, 20, 300.0, 50.0, 100.0, 50.0, 40.0, 15.0)  # 
    print smax, sif, r2
