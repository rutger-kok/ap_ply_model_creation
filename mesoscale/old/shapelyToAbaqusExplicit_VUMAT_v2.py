...
import sigc18 as sigc
import tapePlacement18 as tp
...
#TODO implement chou model to alter the stiffness of undulation regions
...
undMaterial.UserMaterial(
        mechanicalConstants=(72.36428, 5.8058, 5.8058, 0.2692, 0.2692, 0.3515, 3.2288, 
        14.3609, 8.7849, 2.61, 1.759, 0.055, 0.285, 0.105, 53.0, 0.1, 0.1, 0.00075, 
        0.0025, 0.0035))
...
def definePart(obj):
    ...
    objPart.MaterialOrientation(
        ...
        angle=obj.angle, stackDirection=STACK_3)
...
# need to name Polygon objects using their layer and object id
...
for (pathNumber, tapePath) in enumerate(tapePaths):
    if len(tapePath) > 1:
        ...
        pathAngle = partGrid[int(layer)][int(gridID)].angle
