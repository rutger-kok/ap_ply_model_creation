...
def periodicBC(modelName, dimensions, dispVector):
    ...
    # Faces
    matchNodes(modelName, 'T', 'Bm', (0, 1, 0), (-1, 0, 0))
    matchNodes(modelName, 'F', 'Bk', (1, 0, 0), (-1, 0, 0))
    matchNodes(modelName, 'L', 'R', (0, 0, 1), (-1, 0, 0))
    # Edges
    matchNodes(modelName, 'FT', 'BkT', (0, 1, 0), (-1, 0, 0))
    matchNodes(modelName, 'BkT', 'BkBm', (0, 1, 0), (-1, 0, 0))
    matchNodes(modelName, 'BkBm', 'FBm', (1, 0, 0), (1, 0, 0))
    matchNodes(modelName, 'FL', 'BkL', (1, 0, 0), (-1, 0, 0))
    matchNodes(modelName, 'BkL', 'BkR', (0, 0, 1), (-1, 0, 0))
    matchNodes(modelName, 'BkR', 'FR', (1, 0, 0), (1, 0, 0))
    matchNodes(modelName, 'TL', 'BmL', (0, 1, 0), (-1, 0, 0))
    matchNodes(modelName, 'BmL', 'BmR', (0, 0, 1), (-1, 0, 0))
    matchNodes(modelName, 'BmR', 'TR', (0, 1, 0), (1, 0, 0))
    # Vertices
    matchNodes(modelName, 'C6', 'C2', (0, 1, 0), (1, 0, 0))
    matchNodes(modelName, 'C2', 'C3', (0, 0, 1), (-1, 0, 0))
    matchNodes(modelName, 'C3', 'C4', (1, 0, 0), (1, 0, 0))
    matchNodes(modelName, 'C4', 'C8', (0, 1, 0), (-1, 0, 0))
    matchNodes(modelName, 'C8', 'C5', (0, 0, 1), (1, 0, 0))
    matchNodes(modelName, 'C5', 'C1', (0, 1, 0), (1, 0, 0))
    ...
    for ind, s in enumerate(dispVector):
        if s:
            ...
            activeModel.DisplacementBC(amplitude=UNSET,
                                       ...
                                       fixed=OFF, localCsys=None,
                                       name='BC-{}'.format(ind),
                                       region=masterNodeRegion1, u1=dis[0],
                                       u2=dis[1], u3=dis[2], ur1=dis[3],
                                       ur2=dis[4], ur3=dis[5])
...
def createNodeSet(assembly, setName, xmin, ymin, zmin, xmax, ymax, zmax,
    ...
    inst = assembly.instances['MergedPart-1']
...
def createVertexSet(assembly, setName, x, y, z):
    inst = assembly.instances['MergedPart-1']
    findVertex = [node.label for node in inst.nodes
                  if isClose(node.coordinates, (x, y, z))]
    nodeSequence = inst.nodes.sequenceFromLabels(((findVertex[0]), ), )
    vertexSet = assembly.Set(nodes=nodeSequence, name=setName)
...
def isClose(array1, array2, tol=1e-6):
    '''
    Checks whether all the elements in two arrays are the same witin a
    specified tolerance.
    Same as numpy.isclose or math.isclose but these are not available in the
    Python packages that Abaqus uses.
    '''
    boolArray = []
    try:
        for idx, elem1 in enumerate(array1):
            elem2 = array2[idx]
            delta = abs(elem1 - elem2)
            if delta <= tol:
                boolArray.append(True)
            else:
                boolArray.append(False)
        return all(boolArray)
    except IndexError:
        return False
